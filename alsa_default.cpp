...
void switchDevice(alsa_handle_t *handle, uint32_t devices, uint32_t mode)
{
    ...
    if (mode == AudioSystem::MODE_IN_CALL && platform_is_Fusion3() && (inCallDevSwitch == true)) {
       ...
       /* get rx acdb id */
        ...
        } else if (rx_dev_id == DEVICE_SPEAKER_MONO_RX_ACDB_ID &&
...
{
    int err = 0;
    LOGD("s_set_sglte_volume: volume %d", vol);
    ALSAControl control("/dev/snd/controlC0");
    ...
    if (platform_is_Fusion3()) {
        err = csd_client_volume(vol);
        if (err < 0) {
            LOGE("s_set_sglte_volume: csd_client error %d", err);
        }
    }
}
...
{
    int err = 0;
    LOGD("s_set_sglte_mic_mute: state %d", state);
    ALSAControl control("/dev/snd/controlC0");
    ...
    if (platform_is_Fusion3()) {
        err = csd_client_mic_mute(state);
        if (err < 0) {
            LOGE("s_set_sglte_mic_mute: csd_client error %d", err);
        }
    }
}
