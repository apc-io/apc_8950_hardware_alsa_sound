...
status_t AudioHardwareALSA::setVoiceVolume(float volume)
{
    ...
    if (mMixer)
        return mMixer->setVolume(AudioSystem::ROUTE_EARPIECE, volume, volume);
    else
        ...
        status_t *status)
{
    ...
    alsa_handle_t *h = &(*mDeviceList.begin());

    err = mALSADevice->open(h, AudioSystem::ROUTE_SPEAKER, mode());
    if (!err) {
        out = new AudioStreamOutALSA(this, h);
        mOutput = out;
        err = out->set(format, channelCount, sampleRate);
    }
        ...
        AudioSystem::audio_in_acoustics acoustics)
{
    ...
    alsa_handle_t *h = &(*mDeviceList.end());

    err = mALSADevice->open(h, 0, mode());
    if (!err) {
        in = new AudioStreamInALSA(this, h, acoustics);
        mInput = in;
        err = in->set(format, channelCount, sampleRate);
    }
...
{
    if (mMixer)
        return mMixer->setCaptureMuteState(AudioSystem::ROUTE_EARPIECE, state);
...
{
    if (mMixer)
        return mMixer->getCaptureMuteState(AudioSystem::ROUTE_EARPIECE, state);
...
{
    return mALSADevice->route(mOutput->mHandle, mRoutes[mMode], mode());
}
